...
class DropPath(nn.Module):
    ...
    def execute(self, x):
        ...
        random_tensor = jt.rand(x.shape[0], 1, 1, 1)  # Shape (batch_size, 1, 1, 1)
...
class PyramidVisionTransformerImpr(nn.Module):
    ...
    def reset_drop_path(self, drop_path_rate):
        dpr = [x.item() for x in jt.linspace(0, drop_path_rate, sum(self.depths))]
        cur = 0
